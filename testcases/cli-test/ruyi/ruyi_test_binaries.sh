...
function test_oneshot() {
    ...
    for tc in `ruyi list | grep $1 | sed "s/^* $1\///"`; do
        ...
        if [[ "$?"x == "0x" ]] && [[ "$retc"x != "0x" ]]; then
            LOG_WARN "$(gettext "Check $1/$tc no binary for current host")"
            continue
        fi
        if [[ "$retc"x != "0x" ]] || [ -z "$bp" ] || [ ! -d "$bp" ]; then
            CHECK_RESULT $retc 0 0 "$(gettext "Check ruyi install $1/$tc failed")"
            continue
        fi
        if [ ! -d "$bpb" ]; then
            LOG_WARN "$(gettext "Check $1/$tc bin dir failed")"
            continue
        fi
        for b in `ls $bpb`; do
            ...
            [ "$?"x != "0x" ] && continue
            ldd "$bpb"/"$b"
            CHECK_RESULT $? 0 0 "$(gettext "Check ruyi $1/$tc binary $b failed")"
            ldd "$bpb"/"$b" | grep -vE "\(0x" | grep ".so"
            [ "$?"x = "0x" ] && LOG_WARN "$(gettext "Found extra message in ldd $b output")"
            ldd "$bpb"/"$b" | grep -vE "\(0x" | grep ".so" | grep "not found"
            CHECK_RESULT $? 1 0 "$(gettext "Found .so not found in ldd $b output")"
        done
    done
}
